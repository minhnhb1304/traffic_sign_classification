...
def predict_image(image_path: str | Path, model_type: str = "gtsrb", top_k: int = 3):
    """Predicts the label for a single image, returning a list of top-k (class_name, prob)."""
...
def main() -> None:
    ...
    parser.add_argument("--model-type", default="gtsrb", choices=["gtsrb", "vn"],
                        help="Model type: 'gtsrb' (43 classes) or 'vn' (Vietnam traffic signs)")
    parser.add_argument("--top-k", type=int, default=3)
    args = parser.parse_args()

    results = predict_image(args.image, model_type=args.model_type, top_k=args.top_k)
    print(f"\nPrediction results ({args.model_type.upper()}) for: {args.image}")
